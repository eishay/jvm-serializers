...
public class JsonJackson
{
	...
	public static final Serializer<MediaContent> MediaSerializer = new Serializer<MediaContent>()
	{
	  ...
	  private void writeMedia(JsonGenerator generator, Media media) throws IOException
	  {
		...
		generator.writeStringField(FIELD_NAME_TITLE, media.title);
		...
		if (media.hasBitrate) {
			generator.writeNumberField(FIELD_NAME_BITRATE, media.bitrate);
		}
	  ...
	  private void writeImage(JsonGenerator generator, Image image) throws IOException
	  {
		...
		generator.writeStringField(FIELD_NAME_TITLE, image.title);
